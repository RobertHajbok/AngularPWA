...
Since your Angular application is being served on a different port than the API, it will send requests to localhost:4200/api instead of our API which is running on localhost:5000, by default. To achieve this, we need to create a proxy.config.json file and add the following lines:
  ...
  2. Progressive Web App steps (https://www.lynda.com/Angular-tutorials/Angular-Progressive-Web-Apps/590845-2.html) highlighted:
      ...
      ```
      - Create the [manifest.json](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/src/manifest.json) file in the root folder. There are also some sites which can create this file for you from a set options. This is a W3C spec for a website's metadata which defines the app's name, icons and icon names, installed behaviour, themes, colours and splash screens. Make sure you reference this file from index.html, as shown below. Then, in the [.angular-cli.json file](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/.angular-cli.json) and "manifest.json" in the assets array. If you add icons to the manifest file, do not forget to add the folder name to the assets in .angular-cli.json. You can test out these settings on an Android device.
      ``` html
      ...
      ``` html
        ...
        <link rel="apple-touch-icon" href="icons/icon_ios_180.png" sizes="180x180" />
        <link rel="apple-touch-icon" href="icons/icon_ios_120.png" sizes="120x120" />
      ```
      ...
      ```
      - Next step is to add a service worker. In [.angular-cli.json file](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/.angular-cli.json), add the serviceWorker parameter under apps section and set it to true. Install the service-worker package and after a build in production mode you should see some new files in the wwwroot folder of your app (ngsw-manifest.json for example). Note that service workers are disabled in development mode.
      ```
        npm-install --save-dev @angular/service-worker
      ```
      - Use ng serve in production mode to test if the user is given the possibility to install the app after starting it:
      ```
        ng serve --proxy-config proxy.config.json --prod
      ```
      ...
      - If you get a white screen after reloading the page while offline, check if the index.html page was cached. If you type "/index.html" after your URL, the application should be loaded.
      - To solve the problem above, the ngsw-manifest.json file should be modified. In order to do this, we have to create this file in the root folder of our application. By doing this, the generated file will be merged with this one after a build is done. The issue above can be solved with the following JSON in [ngsw-manifest.json](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/ngsw-manifest.json), along with other routing issues:
      ``` json
        {
          "routing": {
            "index": "/index.html",
      ...
      ```
      - You can also remove some assets from caching, for example the icons, as shown below:
      ``` json
        {
          "static.ignore": [
            "^/icons/.*$"
      ...
      ```
      - To cache dynamic content (from your web service for example), add the "dynamic" region to  [ngsw-manifest.json](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/ngsw-manifest.json):
      ``` json
        "dynamic": {
          "group": [
            {
              "name": "api",
              "urls": {
                "http://localhost:4200/api/coffees": {
                  "match": "prefix"
                }
              },
              "cache": {
      ...
      ```
      - If you want your service workers to support application updates, you can use the following piece of code from [app.component.ts file](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/src/app/app.component.ts) in the ngOnInit function:
      ``` typescript
      ...
      ```
      - You can also support push notifications by enabling the push plugin in [ngsw-manifest.json file](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/ngsw-manifest.json) and using the following piece of code from [app.component.ts file](https://github.com/HajbokRobert/AngularPWA/blob/master/AngularPWA/src/app/app.component.ts):
      ``` typescript
